...
from certificate_issuer import issuer
...
class TestCertificateSigner(unittest.TestCase):
    ...
    def test_sign_cert(self):
        ...
        with patch('certificate_issuer.issuer.open', mock_open(read_data='{"assertion":{"uid": "123"}}'), create=True) as m, \
                patch('certificate_issuer.issuer.SignMessage', return_value=b'123'), \
                patch('certificate_issuer.helpers.import_key', return_value='tcKK1A9Si73zG5ZFnA6XYyhAcb1BNrMVyG'):
            issuer.sign_certs(cert_info)
    ...
    def test_hash_certs(self):
        ...
        with patch('certificate_issuer.issuer.open', mock_open(read_data='bibble'.encode('utf-8')), create=True) as m:
            issuer.hash_certs(cert_info)
    ...
    def test__sign_cert(self):
        ...
        with patch('certificate_issuer.issuer.SignMessage', return_value=b'123'):
            signed_cert = issuer.do_sign('{"assertion":{"uid": "123"}}', mock_privkey)
            self.assertEqual(signed_cert, '{"assertion": {"uid": "123"}, "signature": "123"}')

    def test__hash_cert(self):
        hashed_cert = issuer._hash_cert('bibble'.encode('utf-8'))
